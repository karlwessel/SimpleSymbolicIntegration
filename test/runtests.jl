...
@testset "generic" begin
    x1, y1, a1 = @syms x1 y1 a1
    x2, y2, a2 = @variables x2 y2 a2
    for (T, x, y, a) in [
            (SymbolicUtils.BasicSymbolic{Number}, x1, y1, a1),
            (Num, x2, y2, a2)]
...
@testset "Symbolics" begin
    ...
    @test repr(intx0to2(1/sin(x))) == "Integral(x, 0 .. 2)(1 / sin(x))"
    @test repr(intx0to2(sin(x)/x)) == "Integral(x, 0 .. 2)(sin(x) / x)"
